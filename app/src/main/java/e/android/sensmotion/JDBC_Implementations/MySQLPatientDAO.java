...
public class MySQLPatientDAO implements PatientDAO {
    ...
    public List<PatientDTO> getPatienter() throws DALException {
        ...
        try
        {
            while (rs.next())
            {
                List.add(new PatientDTO(rs.getString("CPR"), rs.getArray("Sensor"), rs.getString("Mobilitet"), rs.getString("Projekt"), rs.getString("Terapuet"), rs.getString("Navn"), rs.getBoolean("aktiv")));
            }
    ...
    public PatientDTO getPatient(String CPR) throws DALException {
        ...
        try {
            if (!rs.first()) throw new DALException("Patienten med CPR-nummeret " + CPR + ", findes ikke");
            return new PatientDTO(rs.getString("CPR"), rs.getArray("Sensor"), rs.getString("Mobilitet"), rs.getString("Projekt"), rs.getString("Terapuet"), rs.getString("Navn"), rs.getBoolean("aktiv"));
        }
    ...
    public void opretPatient(PatientDTO Patient) throws DALException {
        Connector.getInstance().doUpdate(
                "INSERT INTO raavare(raavare_id, raavare_navn, leverandoer) VALUES " +
                        "(" + Patient.getCPR() + ", '" + Patient.getSensorer() + "', '" + Patient.getMobilitet() + "', '"
                        + Patient.getProjekt() + "', '" + Patient.getTerapeut() + "', '" + Patient.getNavn() + "', '"
                        + Patient.isActive() + "')"
        );
    }
}
